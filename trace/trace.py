...
def create_network(inpt, out, learning_rate=0.001):
    class UNet:
        ...
        W_conv2 = weight_variable([3, 3, 1, 48])
        ...
        W_conv10 = weight_variable([3, 3, 48, 200])
        b_conv10 = bias_variable([200])
        h_conv10 = tf.nn.relu(conv2d(h_conv9, W_conv10, dilation=8  ) + b_conv10)

        # layer 11  - 3x3 convolution  
        W_conv11 = weight_variable([3, 3, 200, 200])
        b_conv11 = bias_variable([200])
        # use cropped output from convolution
        ...
        W_conv13 = weight_variable([3, 3, 200, 400])
        b_conv13 = bias_variable([400])
        h_conv13 = tf.nn.relu(conv2d(h_conv12, W_conv13, dilation=16) + b_conv13)

        # layer 14  - 3x3 convolution  
        W_conv14 = weight_variable([3, 3, 400, 400])
        b_conv14 = bias_variable([400])
        ...
        W_conv15 = weight_variable([2, 2, 400, 400])
        b_conv15 = bias_variable([400])
        h_conv15 = tf.nn.relu(deconv2d(h_conv14, W_conv15, [400, 400]) + b_conv15)

        # layer 15.5 - combine upsampling with Step 11.5
        comb_conv15 = crop_and_concat(h_conv11, h_conv15, [16])

        # layer 16   - 3x3 convolution  
        W_conv16 = weight_variable([3, 3, 400, 200])
        b_conv16 = bias_variable([200])
        h_conv16 = tf.nn.relu(conv2d(comb_conv15, W_conv16, dilation=8) + b_conv16)

        # layer 17  - 3x3 convolution  
        W_conv17 = weight_variable([3, 3, 200, 200])
        b_conv17 = bias_variable([200])
        h_conv17 = tf.nn.relu(conv2d(h_conv16, W_conv17, dilation=8) + b_conv17)
        
        # layer 18   - up-sample 2x2 convolution  
        W_conv18 = weight_variable([2, 2, 200, 200])
        b_conv18 = bias_variable([200])
        h_conv18 = tf.nn.relu(deconv2d(h_conv17, W_conv18) + b_conv18)

        # layer 18.5 - combine upsampling with Step 8.5
        comb_conv18 = crop_and_concat(h_conv8, h_conv18, [32])

        # layer 19   - 3x3 convolution  
        W_conv19 = weight_variable([3, 3, 400, 200])
        b_conv19 = bias_variable([200])
        h_conv19 = tf.nn.relu(conv2d(comb_conv18, W_conv19, dilation=4) + b_conv19)

        # layer 20  - 3x3 convolution  
        W_conv20 = weight_variable([3, 3, 200, 200])
        b_conv20 = bias_variable([200])
        h_conv20 = tf.nn.relu(conv2d(h_conv19, W_conv20, dilation=4) + b_conv20)
        
        # layer 21   - up-sample 2x2 convolution  
        W_conv21 = weight_variable([2, 2, 200, 200])
        b_conv21 = bias_variable([200])
        h_conv21 = tf.nn.relu(deconv2d(h_conv20, W_conv21) + b_conv21)

        # layer 21.5 - combine upsampling with Step 5.5
        comb_conv21 = crop_and_concat(h_conv5, h_conv21, [64])

        # layer 22   - 3x3 convolution  
        W_conv22 = weight_variable([3, 3, 200, 48])
        ...
        W_conv24 = weight_variable([2, 2, 48, 48])
        b_conv24 = bias_variable([48])
        h_conv24 = tf.nn.relu(deconv2d(h_conv23, W_conv24) + b_conv24)

        # layer 24.5 - combine upsampling with Step 2.5
        comb_conv24 = crop_and_concat(h_conv2, h_conv24, [128])

        # layer 25   - 3x3 convolution  
        W_conv25 = weight_variable([3, 3, 48, 24])
        b_conv25 = bias_variable([24])
        h_conv25 = tf.nn.relu(conv2d(comb_conv24, W_conv25, dilation=1) + b_conv25)

        # layer 26  - 3x3 convolution  
        W_conv26 = weight_variable([3, 3, 24, 24])
