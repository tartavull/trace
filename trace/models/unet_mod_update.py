...
UNET_MOD_NOMAX = UNetModArchitecture(
    model_name='unet_mod_nomax',
    output_mode=AFFINITIES_3D,
    #layers=[],
)
...
class UNet_Mod(Model):
    def __init__(self, architecture, is_training=False):
        ...
        l0_f1 = tf.nn.elu(same_conv3d(l0, w0_f1)+ b0_f1)

        w0_f2 = get_weight_variable("w0_f2", [3,3,3,64,64])
        b0_f2 = get_bias_variable("b0_f2", [64])
        l0_f2 = tf.nn.elu(same_conv3d(l0_f1, w0_f2)+ b0_f2)

        l1 = max_pool_3d(l0_f2)
        #w_l1 = get_weight_variable("w_l1",[2,2,2,64,128])
        #b_l1 = get_bias_variable("b_l1",[128])
        #l1 = tf.nn.elu(down_conv3d(l0_f2,w_l1)+b_l1)

        w1_f1 = get_weight_variable("w1_f1", [3,3,3,64,128])
        b1_f1 = get_bias_variable("b1_f1", [128])
        l1_f1 = tf.nn.elu(same_conv3d(l1, w1_f1)+ b1_f1)

        w1_f2 = get_weight_variable("w1_f2", [3,3,3,128,128])
        b1_f2 = get_bias_variable("b1_f2", [128])
        l1_f2 = tf.nn.elu(same_conv3d(l1_f1, w1_f2)+ b1_f2)

        l2 = max_pool_3d(l1_f2)
        #w_l2 = get_weight_variable("w_l2",[2,2,2,128,256])
        #b_l2 = get_bias_variable("b_l2",[256])
        #l2 = tf.nn.elu(down_conv3d(l1_f2,w_l2)+b_l2)


        w2_f1 = get_weight_variable("w2_f1", [3,3,3,128,256])
        b2_f1 = get_bias_variable("b2_f1", [256])
        l2_f1 = tf.nn.elu(same_conv3d(l2, w2_f1)+ b2_f1)

        w2_f2 = get_weight_variable("w2_f2", [3,3,3,256,256])
        b2_f2 = get_bias_variable("b2_f2", [256])
        l2_f2 = tf.nn.elu(same_conv3d(l2_f1, w2_f2)+ b2_f2)

        l3 = max_pool_3d(l2_f2)
        #w_l3 = get_weight_variable("w_l3",[2,2,2,256,512])
        #b_l3 = get_bias_variable("b_l3",[512])
        #l3 = tf.nn.elu(down_conv3d(l2_f2,w_l3)+b_l3)


        w3_f1 = get_weight_variable("w3_f1", [3,3,3,256,512])
        b3_f1 = get_bias_variable("b3_f1", [512])
        l3_f1 = tf.nn.elu(same_conv3d(l3, w3_f1)+ b3_f1)

        w3_f2 = get_weight_variable("w3_f2", [3,3,3,512,512])
        b3_f2 = get_bias_variable("b3_f2", [512])
        l3_f2 = tf.nn.elu(same_conv3d(l3_f1, w3_f2)+ b3_f2)

        l4 = max_pool_3d(l3_f2)
        #w_l4 = get_weight_variable("w_l4",[2,2,2,512,1024])
        #b_l4 = get_bias_variable("b_l4",[1024])
        #l4 = tf.nn.elu(down_conv3d(l3_f2,w_l4)+b_l4)
...
class UNet_Mod_nomax(Model):
    def __init__(self, architecture, is_training=False):
        super(UNet_Mod_nomax, self).__init__(architecture)
        prev_layer = self.image
        prev_n_feature_maps = 1
        ...
        w1_f1 = get_weight_variable("w1_f1", [3,3,3,128,128])
        b1_f1 = get_bias_variable("b1_f1", [128])
        l1_f1 = tf.nn.elu(same_conv3d(l1, w1_f1)+ b1_f1)
        ...
        w2_f1 = get_weight_variable("w2_f1", [3,3,3,256,256])
        b2_f1 = get_bias_variable("b2_f1", [256])
        l2_f1 = tf.nn.elu(same_conv3d(l2, w2_f1)+ b2_f1)
        ...
        b_l3 = get_bias_variable("b_l3",[512])
        l3 = tf.nn.elu(down_conv3d(l2_f2,w_l3)+b_l3)
        w3_f1 = get_weight_variable("w3_f1", [3,3,3,512,512])
        b3_f1 = get_bias_variable("b3_f1", [512])
        l3_f1 = tf.nn.elu(same_conv3d(l3, w3_f1)+ b3_f1)
        ...
        w4_f1 = get_weight_variable("w4_f1", [3,3,3,1024,1024])
        b4_f1 = get_bias_variable("b4_f1", [1024])
        l4_f1 = tf.nn.elu(same_conv3d(l4, w4_f1)+ b4_f1)
