...
class CREMIDataset(Dataset):
    ...
    def __init__(self, data_folder, task):
        ...
        if task == down.BOUNDARY:
            ...
            second_smallest = np.partition(filter(lambda a: a != 0, self.train_labels), 0, axis=None)[0]
        ...
        if task == down.BOUNDARY:
            ...
            second_smallest = np.partition(filter(lambda a: a != 0, self.validation_labels), 0, axis=None)[0]
